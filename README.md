...
교안은 [`docs/data_handling/`](docs/data_handling/), 노트북은 [`notebooks/data_handling/`](notebooks/data_handling/)에 있습니다. **(20~22 완료 ✅)**

- [20 numpy](docs/data_handling/20_numpy.md) — 벡터 연산, 불리언 인덱싱, axis, 브로드캐스팅
- [21 pandas](docs/data_handling/21_pandas.md) — 데이터프레임, `groupby`/`merge` (SQL과 1:1 대조)
- [22 시각화](docs/data_handling/22_visualization.md) — matplotlib/seaborn, 차트 선택, EDA

### 4. ML 입문 (scikit-learn)

라이브러리별/개념별로 **노트북 1개씩** 진행합니다.
교안은 [`docs/ml/`](docs/ml/), 노트북은 [`notebooks/ml/`](notebooks/ml/)에 있습니다. **(30~33 완료 ✅)**

- [30 분류 기초](docs/ml/30_classification.md) — ML 4단계 흐름(분할→fit→predict→평가), KNN/결정트리, iris
- [31 회귀](docs/ml/31_regression.md) — 연속값 예측, 선형회귀, 평가 지표(MSE/R²)
- [32 전처리 / 파이프라인](docs/ml/32_preprocessing.md) — 스케일링·인코딩, `Pipeline`(누수 방지)
- [33 모델 평가 / 교차검증](docs/ml/33_evaluation.md) — 혼동행렬, 정밀도/재현율, `cross_val_score`
